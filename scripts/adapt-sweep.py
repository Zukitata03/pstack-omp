...
SUBS = [
    ...
    (r"Always `environment: \"cloud\"` unless the task needs this machine: `control-ui` or `control-cli` runtime verification \(from `cursor-team-kit`\)(?:;|\.) [Rr]eading local transcripts under `agent-transcripts/`(?:;|\.) [Ss]imulators and local IDE state(?:;|\.) [Aa]uth that exists only here(?:;|\.)? Cloud agents cannot read the local store, so their briefs inline what they need or point at repo paths\.",
     "Prefer isolated worktree spawns unless the task needs this machine's live state: runtime verification on the real surface, reading local transcripts, simulators and local IDE state, auth that exists only here. All agents run on this machine, so briefs can point at repo paths and `local://` URIs freely."),
    ...
    (r"`claude-fable-5-1-thinking-max`, `gpt-5.6-sol-max`, `grok-4.6-fast-xhigh`, `claude-opus-5-thinking-xhigh`", "distinct roles for diversity, `slow` seats plus one `advisor` seat"),
    ...
    (r"`claude-fable-5-1-thinking-max`", "the `slow` role"),
    ...
    (r"One message, three `Task` calls, `subagent_type: generalPurpose`, explicit `model:` on each, agent mode \(`readonly: false`\)\.",
     "One `tasks[]` batch with three items, `agent: \"task\"`, an explicit model role on each, full tool access including MCP."),
    (r"Reviewers need MCP access for context lookups \(tickets, chat threads, observability traces referenced in the transcript\)(?:;|\.) [Rr]eadonly strips MCPs\.",
     "Reviewers need MCP access for context lookups (tickets, chat threads, observability traces referenced in the transcript)."),
    ...
    (r"bug-fix: claude-fable-5-1-thinking-max", "bug-fix: default"),
    (r"perf-issue: claude-fable-5-1-thinking-max", "perf-issue: default"),
    (r"hillclimb: claude-fable-5-1-thinking-max", "hillclimb: default"),
    (r"judgment and prose: claude-fable-5-1-thinking-max", "judgment and prose: slow"),
    (r"hardest tasks: claude-fable-5-1-thinking-max", "hardest tasks: slow"),
    (r"how explainer: claude-fable-5-1-thinking-max", "how explainer: slow"),
    (r"why synthesizer: claude-fable-5-1-thinking-max", "why synthesizer: slow"),
    (r"reflect judgment, divergent, synthesizer: claude-fable-5-1-thinking-max", "reflect judgment, divergent, synthesizer: slow"),
    (r"how critics: claude-fable-5-1-thinking-max, gpt-5\.6-sol-max, grok-4\.6-fast-xhigh, claude-opus-5-thinking-xhigh", "how critics: slow, slow, advisor, smol"),
    (r"arena runners: claude-fable-5-1-thinking-max, gpt-5\.6-sol-max, grok-4\.6-fast-xhigh, claude-opus-5-thinking-xhigh", "arena runners: slow, slow, advisor, smol"),
    (r"architect runners: claude-fable-5-1-thinking-max, gpt-5\.6-sol-max, grok-4\.6-fast-xhigh, claude-opus-5-thinking-xhigh", "architect runners: slow, slow, advisor, smol"),
    (r"interrogate reviewers: claude-fable-5-1-thinking-max, gpt-5\.6-sol-max, grok-4\.6-fast-xhigh, claude-opus-5-thinking-xhigh", "interrogate reviewers: slow, slow, advisor, smol"),
    (r"arena cross-judge pool: claude-fable-5-1-thinking-max, gpt-5\.6-sol-max, grok-4\.6-fast-xhigh, claude-opus-5-thinking-xhigh", "arena cross-judge pool: advisor, smol"),
    ...
    (r"\*\*Defaults for every `Task` call\.\*\*(?s:.*?)fast code model\.", "**Defaults for every task batch item.** Jobs run in background automatically, full tool access (never strip MCP from a reviewer), file pointers not inlined context, explicit model role per seat per `references/omp-tools.md`. Code delegates tier by difficulty. The hardest changes get the `slow` role when the task needs judgment or the intent is vague; a precisely specified sequence of steps gets the `default` role; trivial mechanical edits go to `smol`."),

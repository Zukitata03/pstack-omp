...
Look recursively for `the user's OMP skills directory/**/*-mode/SKILL.md` and `~/the user's OMP skills directory/*-mode/SKILL.md` matching the user's handle. Mode skills can live in a personal category directory (`the user's OMP skills directory/<handle>/`), not only at the top level. If one exists, confirm intent with `ask` (unless they already said "update my skill" or similar):
...
Locate the active workspace's transcripts before fanning out. The system prompt names the workspace's `agent-transcripts/` directory. Use only that path. Don't glob across session storage. That crosses workspace boundaries and reads private chats from unrelated projects.
...
- Path: preserve an existing mode skill's category. For a new mode, use `the user's OMP skills directory/<handle>/<handle>-mode/SKILL.md` when the repo has an established personal category for that handle; otherwise default to `the user's OMP skills directory/<handle>-mode/SKILL.md` in the project (or `~/the user's OMP skills directory/<handle>-mode/` if the user prefers a personal skill).
...
- The harness's `skill-creator` skill: skill authoring process and writing guidelines.

...
2. Pick the wake mechanism using OMP's loop. Arm it via `hub op:"start"` with a detached, restart-always process; a watcher subagent wakes you on the event, with a long time-based heartbeat as fallback. An event to watch (CI, a merge, a ref advancing) gets a watcher subagent that wakes you on the event, with a long time-based heartbeat as fallback. No event gets a fixed-interval heartbeat sized to when the result is worth re-checking.

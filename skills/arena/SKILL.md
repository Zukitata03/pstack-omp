...
3. Pick the runners. Use `arena runners` from the user's pstack model config when present. Otherwise default to one each on distinct model roles for diversity, `slow` seats plus one `advisor` seat. Spawn more when the arena covers multiple design directions. Same model N times when the work is generation-bound rather than judgment-sensitive.

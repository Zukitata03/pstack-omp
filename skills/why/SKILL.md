...
- `agent`: `task`
- model role: `smol`
- full tool access including MCP. Investigators still shouldn't write anything. That's a posture, not a sandbox.
...
- `agent`: `task`
- model role: `slow`
- full tool access. The synthesizer's quality check spot-verifies citations, which can require MCP access.
